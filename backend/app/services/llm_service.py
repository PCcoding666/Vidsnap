...
class QwenVLService:
    ...
    async def compose_illustrated_note(
        ...
        prompt = (
            ...
            f"视频转录全文（每行以 [mm:ss] 时间戳开头，标记该句在视频中的起点）：\n{transcript_text}\n\n"
            "要求：\n"
            "1. 用 markdown 写，按【主题】用 ## 小标题组织（如概念、流程、要点），"
            "围绕内容逻辑而非逐句复述转录。\n"
            "2. 每个 ## 小标题必须锚定时间戳，格式 `## 标题 [mm:ss]`：时间戳直接取自上面转录里"
            "真实出现的 [mm:ss]，就近选该主题第一次出现的那一行，绝不自行编造或推算。\n"
            "3. 讲到某个主题时，如果某张关键帧能直接说明它，就在该处单独一行写 [[FRAME:F2]]"
            "（用对应编号；只在真正相关处插入；同一帧最多用一次；不相关的帧不要用）。\n"
            "4. 如果某个重要概念或流程没有合适的关键帧，但用示意图能讲得更清楚，"
            "就直接生成一个简洁正确的 mermaid 代码块（```mermaid 开头）来图解它。\n"
            "5. 笔记要忠于转录内容，不要编造视频里没有的事实。\n"
            "6. 只输出 markdown 笔记正文，不要额外说明。"
        )
