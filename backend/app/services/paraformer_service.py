...
实际使用的模型由 settings.ASR_MODEL 决定（默认 Fun-ASR）；类名/文件名沿用
...
class ParaformerSpeechService:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            ...
            self.available = True
            logger.info("阿里云 Fun-ASR 语音服务初始化成功")
    ...
    async def transcribe_audio_with_timestamps(
        ...
        """
        使用阿里云 Fun-ASR API 转录音频并获取详细时间戳
        ...
        try:
            ...
            logger.info("发送转录请求到阿里云 Fun-ASR...")
