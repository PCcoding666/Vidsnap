...
| 音频转录 | OpenAI Whisper API | 高质量音频转录，段落级时间戳 |
| 视频下载 | yt-dlp | YouTube视频下载 |
| 云存储 | 阿里云OSS | 统一媒体文件存储 |
| 多模态AI | DashScope API | 智能交互分析 |
| 数据库 | SQLite/本地文件 | 元数据存储 |
...
class AudioTranscriptionService:
    ...
    async def extract_and_transcribe_audio(self, video_path: str, video_id: str) -> TranscriptionResult:
        ...
        # 调用OpenAI Whisper进行转录
        transcription = await self.whisper_client.transcribe_file(
            audio_path,
...
pip install alibabacloud-oss2 openai
...
### 9.1 技术选型确认
- [x] 使用OpenAI Whisper API替代阿里云智能语音服务
- [x] 保持Gradio作为前端界面框架
- [x] 使用阿里云OSS进行媒体文件存储
- [x] 使用yt-dlp进行YouTube视频下载
- [x] 使用ffmpeg进行音视频处理

### 9.2 功能实现确认
- [x] 双输入源支持（上传文件 + YouTube URL）
- [x] 并行处理音视频（异步执行）
- [x] 场景变化检测提取关键帧（最多10帧）
- [x] OpenAI Whisper音频转录（段落级时间戳）
- [x] 阿里云OSS存储集成
- [x] Gradio交互界面

### 9.3 项目结构确认
- [x] 保留核心服务文件：
  - `aliyun_video_service.py`（视频下载和关键帧提取）
  - `openai_speech_service.py`（音频转录）
  - `aliyun_oss_service.py`（OSS存储）
  - `aliyun_video_pipeline.py`（协调视频处理流程）
  - `aliyun_gradio_app.py`（Gradio界面）
